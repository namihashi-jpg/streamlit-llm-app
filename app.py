...
company_size = st.selectbox(
    "企業規模",
    ["1〜49名", "50〜299名", "300〜999名", "1,000～5,000名", "5,001名以上"]
)
role = st.selectbox(
    "入力者の立場",
...
if st.button("診断する"):
    ...
    st.subheader("レポート要約")
    summary_lines = [
        f"会社名：{company_name if company_name else '未入力'}",
        f"業種：{industry}",
        f"企業規模：{company_size}",
        f"入力者：{role}",
        f"SCS自己判定：{star_result}",
        f"総合スコア：{total_score} / {max_score}（達成率 {score_ratio}%）",
    ]
